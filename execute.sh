...
echo ''
read -n 1 -p "-> Inserting now the origin for user-00 that arrives late $old_dt"
tb datasource append ds_origins /tmp/origin_first.csv 1>/dev/null
tb sql "select * from visits_joined"
echo '* The country is updated with the old information'

echo ''
read -n 1 -p '-> Inserting now a new origin for user-01'
dt=$(date '+%Y/%m/%d %H:%M:%S')
...
echo ''
read -n 1 -p '-> And now the visit for user-01 that arrives after the origin we just inserted'
